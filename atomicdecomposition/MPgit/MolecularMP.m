...
function mbook = MolecularMP(sc,ip,fftlen,window)
    ...
    % ind1 = 8192*5+30;
     ...
     %start building a molecule with this seed atom
      mbook = buildmolecule(D,res,window,book,bin,dicblk,scale,fftlen,mms);
end
...
%generate explicit dictionary
 ...
 function mbook = buildmolecule(D,res,window,book,bin,dicblk,scale,fftlen,mms)
 ...
 while (done==0)
    ...
    %       tom = [pppad;at;poipad];
         ...
         % atom is too low or 5 atoms are selected 
          buildmolecule(Dm,res,window,book,bin,dicblk2,scale,fftlen,mms);
          Dm=[];
